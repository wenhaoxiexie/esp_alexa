...
// extern wifi_prov_config_handlers_t  wifi_prov_handlers;      //lin
extern wifi_prov_config_handlers_t  wifi_prov_handlers_gosund;
...
static esp_err_t app_prov_start_service(void)
{
    ...
    if (protocomm_add_endpoint(g_prov->pc, "prov-config",
                               ...
                               (void *) &wifi_prov_handlers_gosund) != ESP_OK) {
...
esp_err_t app_prov_event_handler(void *ctx, system_event_t *event)
{
    ...
    case SYSTEM_EVENT_STA_DISCONNECTED:
        ...
        case WIFI_REASON_HANDSHAKE_TIMEOUT:
            // ESP_LOGI(TAG, "STA Auth Error:%d",i);
            printf("app_prov :STA Auth Error:%d\r\n",i);
            i++;
            if(i>=5){
