...
IRAM_ATTR esp_err_t va_led_set(int va_state)      //0001 1001
{
    if (!init_done) {
        return ESP_OK;
    }
    if(va_state == VA_MUTE_ENABLE) {             //0001 0011  19
...
static void stm8_led_send_vl(va_led_specs_t *va_led_v, uint8_t num)
{
    if((*va_led_v[num].va_led_val!=0x123456)||(*va_led_v[num].va_led_val!=0x654321)){
        xSemaphoreTake(led_st.va_led_tim_sema, portMAX_DELAY);
        stm8_led_set_pwm((uint32_t *)va_led_v[num].va_led_val);
        esp_timer_start_once(led_st.esp_delay_timer_hdl, (va_led_v[num].va_led_delay * 1000));
    }
}
//-------------------------------------------------------------------------------------------

static void stm8_led_set_state(size_t sz, const va_led_specs_t *va_led_conf)
{
    for(int i = 0; i < sz; i++) {
        stm8_led_send_vl((va_led_specs_t *)va_led_conf, i);
    }
}


void stm8_ply_set_state(bool status)
{
    static uint32_t stm8_ply_status;
    if(status){
        stm8_ply_status = 0X123456;
    }else{
        stm8_ply_status = 0X654321;
    }
    
    stm8_led_set_pwm(&stm8_ply_status);
}
//-------------------------------------------------------------------------------------------

static void va_stm8_led_task(void *arg)
{
    ...
    while (1) {
        vTaskDelay(1);
        i++;      // Temporary fix for ui-led-thread spinning
        ...
        if (va_led_priority[0].va_led_priority_status) {
            if (!(va_led_con[VA_LED_MIC_OFF_START].va_led_state_st->loop_en)) {
              stm8_led_set_state(va_led_con[VA_LED_MIC_OFF_START].va_led_state_sz, va_led_con[VA_LED_MIC_OFF_START].va_led_state_st);
            }
            if (!(va_led_con[VA_LED_MIC_OFF_ON].va_led_state_st->loop_en)) {
                stm8_led_set_state(va_led_con[VA_LED_MIC_OFF_ON].va_led_state_sz, va_led_con[VA_LED_MIC_OFF_ON].va_led_state_st);
            }
            va_led_is_mute = true;
            va_led_priority[0].va_led_priority_status = false;
        ...
        if (va_led_priority[1].va_led_priority_status) {
            if (!(va_led_con[VA_LED_MIC_OFF_END].va_led_state_st->loop_en)) {
                stm8_led_set_state(va_led_con[VA_LED_MIC_OFF_END].va_led_state_sz, va_led_con[VA_LED_MIC_OFF_END].va_led_state_st);
            }
        ...
        if (va_led_priority[2].va_led_priority_status) {
            if(volume_to_set == 0 || va_led_priority[2].va_led_current_state == VA_SPEAKER_MUTE_ENABLE) {
                for(int i = 0; i < 3; i++) {
                    stm8_led_set_state(va_led_con[VA_LED_SPEAKER_MUTE].va_led_state_sz, va_led_con[VA_LED_SPEAKER_MUTE].va_led_state_st);
                }
            } else {
                ...
                stm8_led_send_vl(va_led_con[VA_LED_SPEAKER_VOL].va_led_state_st, vol - 1);
        ...
        if (va_led_priority[3].va_led_priority_status) {
            switch (va_led_priority[3].va_led_current_state) {
                case VA_CAN_START :                                                 //bule led loop lin
                    if (!(va_led_con[VA_LED_BOOTUP_1].va_led_state_st->loop_en)) {
                        stm8_led_set_state(va_led_con[VA_LED_BOOTUP_1].va_led_state_sz, va_led_con[VA_LED_BOOTUP_1].va_led_state_st);  //bootup 1
                    }
                    while (!(va_boot_is_finish()) && !(va_led_priority[3].va_led_current_state == LED_OFF)) {
                        stm8_led_send_vl(va_led_con[VA_LED_BOOTUP_2].va_led_state_st, counter);
                ...
                case VA_IDLE :
                    if(va_led_listening_end_flag && (va_led_priority[3].va_led_current_state == VA_IDLE)) {
                        stm8_led_set_state(va_led_con[VA_LED_WW_DEACTIVATE].va_led_state_sz, va_led_con[VA_LED_WW_DEACTIVATE].va_led_state_st);
                        va_led_listening_end_flag = false;
                        va_led_listen_on_going = false;
                    }
                    if (va_led_dnd_st) {
                        stm8_led_set_state(va_led_con[VA_LED_DND].va_led_state_sz, va_led_con[VA_LED_DND].va_led_state_st);
                        va_led_dnd_st = false;
                    }
                    if (va_led_error_st) {
                        stm8_led_set_state(va_led_con[VA_LED_ERROR].va_led_state_sz, va_led_con[VA_LED_ERROR].va_led_state_st);
                    ...
                    if (NOTIFICATION_IS_PRESENT && (!(ALERT_IS_PRESENT > 0) || va_led_notif_incoming_is_done == true)) {
                        if(va_led_notif_incoming_is_done) {
                            stm8_led_set_state(va_led_con[VA_LED_NTF_INCOMING].va_led_state_sz, va_led_con[VA_LED_NTF_INCOMING].va_led_state_st);
                            va_led_notif_incoming_is_done = false;
                        }
                        if (!(ALERT_IS_PRESENT > 0)) {
                            stm8_led_send_vl(va_led_con[VA_LED_NTF_QUEUED].va_led_state_st, counter);
                            counter = (counter + 1) % va_led_con[VA_LED_NTF_QUEUED].va_led_state_sz;
                            va_led_tick = 0;
                        }
                    } else if (ALERT_IS_PRESENT > 0) {
                        stm8_led_send_vl(va_led_con[VA_LED_ALERT].va_led_state_st, counter);
                        counter = (counter + 1) % va_led_con[VA_LED_ALERT].va_led_state_sz;
                        va_led_tick = 0;
                    } else if(va_led_is_mute) {
                        stm8_led_send_vl(va_led_con[VA_LED_MIC_OFF_ON].va_led_state_st, (va_led_con[VA_LED_MIC_OFF_ON].va_led_state_sz -1));
                        va_led_tick = portMAX_DELAY;
                        break;
                    } else {
                        stm8_led_set_state(va_led_con[VA_LED_OFF].va_led_state_sz, va_led_con[VA_LED_OFF].va_led_state_st);  //LED off
                        va_led_tick = portMAX_DELAY;
                    }
                break;
                case VA_LISTENING :
                    ...
                    if (va_led_listen_on_going == false && !(va_led_con[VA_LED_WW_ACTIVE].va_led_state_st->loop_en)) {
                        stm8_led_set_state(va_led_con[VA_LED_WW_ACTIVE].va_led_state_sz, va_led_con[VA_LED_WW_ACTIVE].va_led_state_st);
                        va_led_listen_on_going = true;
                        va_led_alert_short_en = false;
                    }
                    if (va_led_alert_short_en) {
                        stm8_led_send_vl(va_led_con[VA_LED_ALERT_SHORT].va_led_state_st, counter);
                        if ((counter = (counter + 1) % va_led_con[VA_LED_ALERT_SHORT].va_led_state_sz) == 0) {
                            va_led_alert_short_en = false;
                        }
                        va_led_tick = 0;
                    } else {
                        stm8_led_send_vl(va_led_con[VA_LED_WW_ONGOING].va_led_state_st, 0);
                        va_led_tick = portMAX_DELAY;
                    }
                break;
                case VA_THINKING :
                    ...
                    if (va_led_alert_short_en) {
                        ...
                        stm8_led_send_vl(va_led_con[VA_LED_THINKING].va_led_state_st, counter);
                ...
                case VA_SPEAKING :
                    ...
                    if (va_led_alert_short_en) {
                        ...
                        stm8_led_send_vl(va_led_con[VA_LED_SPEAKING].va_led_state_st, counter);
                        counter = (counter + 1) % va_led_con[VA_LED_SPEAKING].va_led_state_sz;

                        stm8_led_set_state(va_led_con[VA_LED_SPEAKING].va_led_state_sz, va_led_con[VA_LED_FACTORY_RESET].va_led_state_st);
                    }
                    va_led_tick = 0;
                    if (DND_IS_PRESENT) {
                        va_led_dnd_st = true;
                    ...
                    break;
                case LED_RESET :
                    stm8_led_set_state(va_led_con[VA_LED_FACTORY_RESET].va_led_state_sz, va_led_con[VA_LED_FACTORY_RESET].va_led_state_st);
                    va_led_tick = 0;
                break;
                case LED_OTA :
                    stm8_led_set_state(va_led_con[VA_LED_OTA].va_led_state_sz, va_led_con[VA_LED_OTA].va_led_state_st);  //led off
                    va_led_tick = portMAX_DELAY;
                break;
                case LED_OFF :
...
esp_err_t va_led_init_stm8(va_led_config_t va_led_conf[VA_LED_PATTERN_MAX])  //lin
{
    ...
    led_st.va_led_task_handle = xTaskCreateStatic(va_stm8_led_task, "ui-led-thread", VA_LED_TASK_STACK_SZ, (void *)va_led_conf,
