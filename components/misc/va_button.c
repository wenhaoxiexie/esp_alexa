...
static void va_button_factory_reset_timer_cb()
{
    ...
    // ESP_LOGI(TAG, "[va_button]:Factory Reset initiated, erasing nvs flash and rebooting");
    printf("[va_button]:Factory Reset initiated, erasing nvs flash and rebooting");
}

void button_factory_reset_bc_3sec()
{
    va_button_factory_reset_timer_cb();
}
...
uint32_t app_to_current_val=5000;
int app_set_volume = 100;
static void va_button_adc_task(void *arg)
{
    //Init ADC and Characteristics
    bool en_timer = 0;
    uint32_t current_adc_val=0;
    uint8_t current_vol;
    int set_volume = 0;
    ...
    while (1) {
        ...
        current_adc_val = app_to_current_val;
        if ((button_st.but_cfg.va_button_adc_val[VA_BUTTON_TAP_TO_TALK] != -1) &&
            ...
            if(button_exe_internal(VA_BUTTON_MIC_MUTE)) {
             ...
             ESP_LOGI(TAG, "-------------------set_volume:%d",app_set_volume);
            if(button_exe_internal(VA_BUTTON_VOLUME_UP)) {
                if(va_boot_is_finish()) {
                    ...
                    ESP_LOGI(TAG, "set_volume:%d",set_volume);
            ...
            if(button_exe_internal(VA_BUTTON_CUSTOM_1)) {
                esp_timer_start_once(button_st.esp_timer_handler_w, (1 * 1000));
            }
        ...
        if(current_adc_val!=5000){
            ESP_LOGI(TAG, "app_to_current_val:%d",app_to_current_val);
            app_to_current_val = 5000;
...
void set_default_volume(void){
    //Init ADC and Characteristics
    media_hal_set_mute(media_hal_get_handle(), 0);
    media_hal_control_volume(media_hal_get_handle(), 100);
}
